...
- [ ] Ручной прогон: киоск → ветвь/лист, тайл, телефон, pre-reg код, success + SMS numpad.
- [ ] `pnpm nx run frontend:lint` (или целевой lint проекта).
- [ ] Нет регрессий i18n: новые подписи админки — через существующие namespaces переводов.
