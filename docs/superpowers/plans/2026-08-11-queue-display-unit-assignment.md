...
- [ ] **Step 1: Add OpenAPI annotations and regenerate artifacts.**
  ...
  Document both response modes and PATCH request semantics, then run `pnpm nx openapi backend` followed by the existing frontend Orval target. Do not hand-edit generated clients.
...
- [ ] **Step 4: Run frontend/shared type checks.**
  ...
  Run: `pnpm nx test shared-types` and `pnpm nx test frontend` with the focused parser test included.
...
- [ ] **Step 3: Run final automated gates.**
  ...
  pnpm nx e2e:experience frontend
  ```
