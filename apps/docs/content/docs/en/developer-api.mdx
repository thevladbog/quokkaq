---
title: Developer API, webhooks, and public queue widget
description: Tenant-facing integrations plan capabilities, IdP, REST, webhooks, and the embeddable queue widget
---
...
When an authenticated tenant admin calls **`GET /companies/me`**, the JSON includes **`planCapabilities`** — booleans derived from the company subscription (SaaS operator tenants may have all features enabled regardless of plan):
...
**Audience:** the team that hosts the **employee or card directory** that must answer _“who is this badge or this login?”_ for a given unit.  
...
**What you must _not_ log in your own systems (recommended):** the raw badge or login in clear text, or the full QuokkaQ user object; keep correlation IDs and outcomes only, in line with your DPA/152‑FЗ obligations.
...
- This documentation site and the in-app `/help` wiki may both describe integrations; the **OpenAPI** spec remains the contract for HTTP.
