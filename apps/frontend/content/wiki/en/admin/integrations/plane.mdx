...
description: Self-hosted Plane as QuokkaQ support backend — deployment, env vars, API check, and troubleshooting.
---
...
- Plane support remains in the codebase. To **turn Plane off** while keeping variables on disk, set `PLANE_ENABLED=false` or choose another backend with `SUPPORT_REPORT_PLATFORM` (see [Yandex Tracker](./yandex-tracker)).
