/**
 * Vendored from [Kibo UI](https://www.kibo-ui.com/components/color-picker) (shadcnblocks/kibo).
 * Local changes: correct controlled-hex sync, safe undefined `value`, avoid spurious onChange
 * (initial render + value-from-parent) which breaks form state; Percentage input uses string values.
 * @see https://github.com/shadcnblocks/kibo/blob/main/packages/color-picker/index.tsx
 */
'use client';
...
export const ColorPicker = ({
  ...
  useEffect(() => {
    ...
    if (skipInitialOnChange.current) {
      skipInitialOnChange.current = false;
      return;
    }
    if (syncFromProp.current) {
      syncFromProp.current = false;
