  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {t('currentPeriod')}: {formatDate(metrics.currentPeriod.start)} - {formatDate(metrics.currentPeriod.end)}
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        {metricsArray.map((metric) => (
          <div key={metric.key} className="space-y-2">
            ...
            {metric.limit === -1 ? (
              <div className="text-xs text-gray-500 italic">{t('unlimited')}</div>
            ) : (
              <>
                <Progress 
                ...
                <div className="flex items-center justify-between text-xs">
                  <span className={metric.percentage >= 90 ? 'text-red-600' : 'text-gray-500'}>
        ...
        {metricsArray.some(m => m.percentage >= 90) && (
          <div className="mt-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-medium text-yellow-900">{t('approachingWarning')}</p>
                <p className="text-sm text-yellow-700 mt-1">{t('approachingDesc')}</p>
              </div>
            </div>
          </div>
        )}
