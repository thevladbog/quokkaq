...
- Base image: golang:1.26.2-alpine for building
...
- **First deployment wizard:** open **`https://<your-app>/{locale}/setup`** after migrations and backing services (Postgres, Redis, MinIO/S3, SMTP) are up. Set **`SETUP_TOKEN`** on the API when **`APP_ENV`** is **`production`** or **`staging`** (required in that case); paste the same value into the wizard so it can call **`GET /system/health`** and **`POST /system/setup`**. The bootstrap step creates the SaaS operator company, a root subdivision, the anonymous kiosk client, and the first user with **`admin`** and **`platform_admin`**. Optional steps cover Yandex Tracker fields (same as **Platform → Integrations**). For existing databases, you can instead grant **`platform_admin`** with **`go run ./cmd/assign-platform-admin -email=user@example.com`** from the backend module with **`DATABASE_URL`** set (see monorepo [SETUP.md](../../../SETUP.md)).
