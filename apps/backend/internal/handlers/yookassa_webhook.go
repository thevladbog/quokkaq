...
func ServeYooKassaWebhook(w http.ResponseWriter, r *http.Request) {
	...
	if secret == "" {
		logger.PrintfCtx(r.Context(), "YooKassa webhook: rejecting request: signing secret is empty (set YOOKASSA_WEBHOOK_SECRET or YOOKASSA_SECRET_KEY)")
	...
	if err := billing.VerifyYooKassaWebhookSignature(body, sig, secret); err != nil {
		logger.PrintfCtx(r.Context(), "YooKassa webhook signature: %v", err)
	...
	if err := billing.HandleYooKassaNotification(ctx, body, func(tx *gorm.DB, invoiceID, paymentID string, paidAt time.Time) error {
		...
		if errors.As(err, &syn) || errors.As(err, &typeErr) {
			logger.PrintfCtx(r.Context(), "YooKassa webhook bad JSON: %v", err)
			http.Error(w, "Bad request", http.StatusBadRequest)
			return
		}
		if errors.Is(err, gorm.ErrRecordNotFound) {
			logger.PrintfCtx(r.Context(), "YooKassa webhook: invoice not found for notification (acknowledging): %v", err)
			w.WriteHeader(http.StatusOK)
			return
		}
		logger.PrintfCtx(r.Context(), "YooKassa webhook: %v", err)
