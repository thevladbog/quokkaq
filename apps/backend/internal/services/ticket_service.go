...
type TicketService interface {
	...
	// documentsData: optional JSON object (document/custom identification); server sets documentsDataExpiresAt from service kiosk settings.
	...
	CreateTicket(unitID, serviceID string, optionalStaffClientID *string, visitorPhone *string, visitorLocale *string, documentsData *json.RawMessage, actorUserID *string, kioskIdentifiedUserID *string) (*models.Ticket, error)
	// CreateTicketWithFunnelOverride is used when the create source must not be inferred (e.g. virtual queue).
	CreateTicketWithFunnelOverride(unitID, serviceID string, optionalStaffClientID *string, visitorPhone *string, visitorLocale *string, documentsData *json.RawMessage, actorUserID *string, funnelSourceOverride *string, kioskIdentifiedUserID *string) (*models.Ticket, error)
...
func (s *ticketService) createTicketInternal(unitID, serviceID string, preRegID *string, optionalStaffClientID *string, visitorPhone *string, visitorLocale *string, documentsData *json.RawMessage, actorUserID *string, isCredit bool, funnelSourceOverride *string, kioskIdentifiedUserID *string) (*models.Ticket, error) {
	...
	if preRegID != nil && HasRequestDocumentsData(documentsData) {
		return nil, errors.New("documentsData cannot be used with pre-registration")
	}
	...
	if err := s.repo.Transaction(func(tx *gorm.DB) error {
		...
		docData, docExp, derr := ResolveDocumentsDataForNewTicket(service, documentsData)
		if derr != nil {
			return derr
		}
	...
	if s.notifService != nil {
		if full, ferr := s.repo.FindByID(ticket.ID); ferr == nil {
			meta := ticketFunnelCreateMetaFromSourceAndTicket(createdSource, full)
			s.notifService.RecordFunnelEvent(full, "ticket_created", "ticketing", meta)
...
// ticketFunnelCreateMetaFromSourceAndTicket enriches queue_funnel analytics (5.1).
func ticketFunnelCreateMetaFromSourceAndTicket(issueSource string, t *models.Ticket) map[string]any {
	if t == nil {
		return nil
	}
	src := strings.TrimSpace(issueSource)
	m := map[string]any{
		"serviceId": t.ServiceID,
		"source":    src,
		"channel":   mapFunnelChannelFromIssueSource(src),
	}
	if t.BookingID != nil && strings.TrimSpace(*t.BookingID) != "" {
		m["fromBooking"] = true
	}
	if t.PreRegistrationID != nil && strings.TrimSpace(*t.PreRegistrationID) != "" {
		m["fromPreRegistration"] = true
	}
	if src == "public_issue_kiosk_phone" || src == "public_issue" || src == "virtual_queue" {
		m["kioskContext"] = true
	}
	return m
}

func mapFunnelChannelFromIssueSource(src string) string {
	switch strings.TrimSpace(src) {
	case "virtual_queue":
		return "virtual_queue"
	case "staff_issue_named":
		return "staff"
	case "public_issue", "public_issue_kiosk_phone":
		return "kiosk"
	case "pre_registration_redeem":
...
func (s *ticketService) UpdateStatus(ticketID, status string, actorUserID *string) (*models.Ticket, error) {
	...
	s.scheduleETA(ticket.UnitID)
	if s.notifService != nil {
		if full, ferr := s.repo.FindByID(ticketID); ferr == nil {
			om := map[string]any{
				"outcome":    status,
			...
			s.notifService.RecordFunnelEvent(full, "ticket_outcome", "ticketing", om)
		}
	}
	return ticket, nil
}
...
func (s *ticketService) VisitorCancelTicket(ticketID string) (*models.Ticket, error) {
	...
	if s.notifService != nil {
		if full, ferr := s.repo.FindByID(ticketID); ferr == nil {
			s.notifService.RecordFunnelEvent(full, "ticket_outcome", "ticketing", map[string]any{
				"outcome":     "no_show",
				"fromStatus":  "waiting",
				"serviceId":   full.ServiceID,
				"visitorExit": "cancel",
