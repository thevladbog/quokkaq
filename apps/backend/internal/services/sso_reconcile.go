...
func (s *SSOService) ApplyPostSSOLogin(
	...
	if extObjectID := strings.TrimSpace(externalObjectID); extObjectID != "" {
		...
		if extErr != nil {
			if errors.Is(extErr, gorm.ErrRecordNotFound) {
				...
				logger.Printf("ApplyPostSSOLogin ssoRepo.FindExternalIdentity(issuer=%q, subject=%q): %v", iss, sub, extErr)
			...
			if ext.ExternalObjectID == nil || strings.TrimSpace(*ext.ExternalObjectID) == "" {
				...
				if err := s.ssoRepo.UpdateExternalIdentity(ctx, ext); err != nil {
					logger.Printf("ApplyPostSSOLogin UpdateExternalIdentity oid: %v", err)
				}
			}
		}
	}
		...
		if emailVerified && strings.TrimSpace(profileEmail) != "" {
			...
			if pe != cur {
				...
				if errors.Is(err, gorm.ErrRecordNotFound) {
					...
					logger.Printf("ApplyPostSSOLogin: skip email update, already taken")
				} else if err != nil {
					logger.Printf("ApplyPostSSOLogin FindByEmail: %v", err)
				}
			}
		}
		if len(updates) > 0 {
			if err := s.userRepo.UpdateFields(ctx, u.ID, updates); err != nil {
				logger.Printf("ApplyPostSSOLogin UpdateFields: %v", err)
			}
		}
	}
	...
	if deferGroupReconcile {
		logger.Printf("ApplyPostSSOLogin: skipping group reconcile (OIDC groups claim omitted due to token overage; use Graph or a later sync)")
		return nil
	}
	...
	defer func() {
		if err != nil {
			if rerr := s.userRepo.RecomputeUserIsActive(ctx, user.ID); rerr != nil {
				logger.Printf("reconcileGroupsToAccess: userRepo.RecomputeUserIsActive(%q) after failed tx: %v", user.ID, rerr)
			}
		}
	}()
