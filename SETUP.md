...
pnpm --dir apps/marketing run orval
```

CI runs `pnpm nx run frontend:orval:check` and `pnpm nx run marketing:orval:check` when those projects (or the backend) are affected.
...
- **Annual prepay (Stripe checkout):** Configure on a catalog plan with `interval: month` only—set either `annualPrepayDiscountPercent` (1–100) or `annualPrepayPricePerMonth` (minor units), never both. Per-unit plans bill `price` (or yearly unit amount) times the count of active `units` with `kind: subdivision` (quantity is at least 1 when there are none). After successful Checkout, the backend merges `preferredBillingPeriod: annual` into `subscriptions.metadata` when the session used annual prepay. Operator-driven Stripe invoice creation uses the same rules via `ManualInvoiceLineAmountMinor`. After changing plan fields in Go, run `pnpm nx run backend:openapi`, then `pnpm nx run frontend:orval` / `marketing:orval`, and validate generated clients with `pnpm nx run openapi:check`, `pnpm nx run frontend:orval:check`, and `pnpm nx run marketing:orval:check` before committing.
